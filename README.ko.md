...
Claude가 다음 구현 slice를 고르거나 capability가 빠졌는지 판단해야 할 때는 가벼운 capability inventory를 포함하세요:
...
- capability가 없다는 주장은 더 높은 증거 기준이 필요합니다. capability-gap analysis에는 `pack-context.sh --inventory`를 사용하고, 로컬 검색으로 확인하기 전까지 missing-feature 결론은 검증되지 않은 가정으로 다루세요. inventory 출력에 `TRUNCATED`가 있으면, 부재 결론을 받아들이기 전에 대상 로컬 검색을 먼저 수행하세요.
